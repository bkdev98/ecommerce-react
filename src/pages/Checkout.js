  ...
  handleSubmit = e => {
    ...
    const messageHtml = `
      <ul>
        ${cartData.map(
          item =>
            `<li>Tên sản phẩm: ${item.product.name} | Số lượng: ${
              item.quantity
            } | Giá tiền: ${item.product.salePrice.toLocaleString()}</li>`
        )}
      </ul>
    `;
    ...
    const templateParams = {
      customer: formFields,
      toName: "Quốc Khánh",
      toEmail: "hoanggia7398@gmail.com",
      fromName: "Leflair",
                ...
                {!cartData.length ? (
                  ...
                  cartData.map(item => (
                    <CartItem key={item.id}>
                      ...
                      <CartContent>
                        <p style={{ margin: 5, fontWeight: 500 }}>
                          {item.product.name}
                        </p>
                        ...
                        <p style={{ margin: 5, fontSize: 14 }}>
                          Giá: {item.product.salePrice.toLocaleString()}₫
                        </p>
                      </CartContent>
                    </CartItem>
                  ))
